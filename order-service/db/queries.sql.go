...
const updateOrderItemStatusByOrderIDDishID = `-- name: UpdateOrderItemStatusByOrderIDDishID :execrows
UPDATE order_items
...
type UpdateOrderItemStatusByOrderIDDishIDParams struct {
	OrderID         string `json:"orderId"`
	DishID          string `json:"dishId"`
	DishOrderStatus string `json:"dishOrderStatus"`
}

func (q *Queries) UpdateOrderItemStatusByOrderIDDishID(ctx context.Context, arg UpdateOrderItemStatusByOrderIDDishIDParams) (int64, error) {
	result, err := q.db.Exec(ctx, updateOrderItemStatusByOrderIDDishID, arg.OrderID, arg.DishID, arg.DishOrderStatus)
	if err != nil {
		return 0, err
	}
